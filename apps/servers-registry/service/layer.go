...
type Layer interface {
	Select(context.Context, uint32, uint32, uint32, uint32) (LayerSelection, error)
...
func (l *layerService) Select(ctx context.Context, realmID, mapID, groupID, forcedLayerID uint32) (LayerSelection, error) {
	...
	if forcedLayerID != 0 {
		return LayerSelection{Status: LayerSelectionOK, Server: leastLoaded(candidates)}, nil
	}
	if groupID == 0 {
		return LayerSelection{Status: LayerSelectionOK, Server: leastLoaded(candidates)}, nil
	}
	...
	if server := serverByID(candidates, boundID); server != nil {
	...
	if server := serverByID(candidates, winner); server != nil {
		return LayerSelection{Status: LayerSelectionOK, Server: server}, nil
	}
	return LayerSelection{Status: LayerSelectionNoServer}, nil
}
...
func (l *layerService) UpdateConfiguration(ctx context.Context, realmID uint32, config map[uint32]uint32) error {
	...
	return l.servers.ConfigureLayers(ctx, realmID, config)
}
...
func (l *layerService) Stats(ctx context.Context, realmID, mapID uint32) (uint32, []LayerStat, error) {
	...
	for _, server := range servers {
		if server.LayerID > 0 {
			stats = append(stats, LayerStat{LayerID: server.LayerID, Players: server.ActiveConnections, Server: server})
		}
	}
	sort.Slice(stats, func(i, j int) bool { return stats[i].LayerID < stats[j].LayerID })
