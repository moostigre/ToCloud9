...
func TestRegistryReplicasShareAtomicGroupBinding(t *testing.T) {
	...
	servers := &layerServersStub{servers: []repo.GameServer{
		{ID: "layer-1", LayerID: 1, ActiveConnections: 2},
		{ID: "layer-2", LayerID: 2, ActiveConnections: 1},
	}}
	...
	wg.Add(2)
	go func() { defer wg.Done(); first, _ = replicaA.Select(context.Background(), 1, 1, 77, 0) }()
	go func() { defer wg.Done(); second, _ = replicaB.Select(context.Background(), 1, 1, 77, 0) }()
	wg.Wait()
...
func TestStaleGroupBindingMovesToAvailableServer(t *testing.T) {
	...
	layers := NewLayer(&layerServersStub{servers: []repo.GameServer{{ID: "ready", LayerID: 2}}}, store)

	selection, err := layers.Select(context.Background(), 1, 1, 77, 0)
	require.NoError(t, err)
...
func TestForcedLayerSelectionDoesNotChangeGroupBinding(t *testing.T) {
	store := newLayerStoreStub()
	require.NoError(t, store.SetConfiguration(context.Background(), 1, map[uint32]uint32{1: 2}))
	require.NoError(t, store.SetGroupBinding(context.Background(), 1, 77, 1, "layer-1"))
	layers := NewLayer(&layerServersStub{servers: []repo.GameServer{{ID: "layer-1", LayerID: 1}, {ID: "layer-2", LayerID: 2}}}, store)

	selection, err := layers.Select(context.Background(), 1, 1, 77, 2)
	require.NoError(t, err)
	require.Equal(t, uint32(2), selection.Server.LayerID)
	bound, err := store.GroupBinding(context.Background(), 1, 77, 1)
	require.NoError(t, err)
