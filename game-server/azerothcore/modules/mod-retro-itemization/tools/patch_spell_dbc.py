...
def main():
    ...
    parser.add_argument("--profile", choices=("tbc",), required=True)
    parser.add_argument("--historical-dbc", required=True)
    parser.add_argument("--profile-sql", required=True)
    ...
    if source_fields != 179 or target_fields != 234:
        raise ValueError(f"expected TBC/WotLK Spell.dbc layouts (179/234), got {source_fields}/{target_fields}")
    ...
    # TBC 2.4.3 physical fields -> WotLK 3.3.5 physical fields. BaseDice and
    # DicePerLevel disappeared in WotLK; BasePoints contains the effective
    # historical amount used by these passive aura records.
    groups = (
        (64, 71),   # Effect
    ...
    text_groups = ((123, 136), (132, 153), (141, 170), (150, 187))
    ...
    print(f"patched {patched} TBC passive item spells; {len(missing)} unavailable records skipped")
    if 34495 in target:
        row = target[34495]
