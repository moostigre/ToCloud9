...
struct MultispecsConfig
{
    ...
    void Load()
    {
        ...
        dualSpecLevel = std::max<uint8>(1, sConfigMgr->GetOption<uint8>("MinDualSpecLevel", 10));
...
{
    // Slot two remains the stock paid trainer purchase. Only extend a player
    // who already owns dual spec into the module-provided third slot.
    if (config.enabled && player && player->GetSpecsCount() == 2 &&
        player->GetLevel() >= config.tripleSpecLevel)
        player->UpdateSpecCount(3);
}
