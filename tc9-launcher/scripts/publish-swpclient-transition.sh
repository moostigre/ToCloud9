...
base_url="https://launcher.expanded.space/downloads/swp-client/files"
...
jq -cn \
    ...
    '{version:$version,launcher:{version:"0.6.0",url:$launcher_url,sha256:$launcher_sha,size:$launcher_size},files:[
      ...
      {path:"Interface/AddOns/SWPClient/SWPClient.toc",url:$toc_url,sha256:$toc_sha,size:$toc_size},
      {path:"Interface/AddOns/SWPClient/SWPClient.lua",url:$lua_url,sha256:$lua_sha,size:$lua_size}
    ]}' > "$payload_file"
...
echo "published SWPClient-to-SWP transition manifest $version to $output_dir"
